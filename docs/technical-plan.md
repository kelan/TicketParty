...
## Proposed Modules
- `TicketCore`: domain models and validation rules.
- `TicketPersistence`: SwiftData models, migrations, query utilities.
- `TicketApp`: SwiftUI screens and view models.
- `TicketCLI`: executable target (`tp`) that reuses `TicketCore` + `TicketPersistence`.
- `TicketDigest`: summary/query engine for "while you were away."
...
- Field naming note: ticket long-form text is conceptually `description`; implementation may store it as `ticketDescription` for SwiftData compatibility.
...
| `Comment` | Structured conversation entry | `id`, `ticketID`, `authorType`, `authorID`, `type`, `body`, `createdAt`, `inReplyToCommentID?`, `requiresResponse`, `resolvedAt?` | Index `ticketID + createdAt`; index `requiresResponse + resolvedAt` |
| `Assignment` | Assignment timeline record | `id`, `ticketID`, `assigneeID`, `assignedBy`, `assignedAt`, `unassignedAt?` | Index `ticketID + assignedAt`; index `assigneeID + unassignedAt` |
...
### Relationship Rules
- `Ticket` to `Note`: one-to-many by `Note.ticketID`.
- `Ticket` to `Comment`: one-to-many by `Comment.ticketID`.
- `Ticket` to `TicketEvent`: one-to-many by `TicketEvent.ticketID`.
- `Ticket` to `TicketStateTransition`: one-to-many by `TicketStateTransition.ticketID`.
- `Ticket` to `Assignment`: one-to-many by `Assignment.ticketID`.
- `Ticket` to `Agent`: many-to-one via `Ticket.assigneeID`.
...
- `Ticket.updatedAt` must change on every mutation to support optimistic concurrency checks.
- `Ticket.displayID` follows format `TT-<ticketNumber>` and is immutable after creation.
- `Ticket.currentState` stores the latest state enum raw value for fast filtering and sorting.
- `Ticket.currentStateChangedAt` tracks when the current state last changed.
- Closing a ticket sets `closedAt`; reopening clears `closedAt`.
- Archiving sets `archivedAt`; archived tickets are excluded from default list queries.
- `Comment.requiresResponse == true` implies unresolved until `resolvedAt` is set.
- Every state change appends one `TicketStateTransition` row and one `TicketEvent` row in the same save transaction as the `Ticket.currentState` update.
- All non-state mutations also append `TicketEvent` rows in the same transaction.

### Current-State Query Strategy
- Current state reads come directly from `Ticket.currentState` and `Ticket.currentStateChangedAt` for O(1)-style row access.
- Historical state analysis reads `TicketStateTransition`, not `Ticket`, to avoid scanning generic events.
- "Latest transition per ticket" query pattern:
- Filter transitions by `ticketID`.
- Sort `transitionedAt DESC`.
- Take first row.

## History Strategy (Forever)
- Append-only `TicketEvent` for all meaningful changes.
- Append-only `TicketStateTransition` for all state changes.
- Never hard-delete ticket records, transition rows, or event rows in normal operation.
- Use soft-delete/archive for hidden records.
- Build digest and audit views from transitions/events + current ticket projection.
- Add indexes for `ticketID`, `timestamp`, `eventType`, `toState`, `requiresResponse`.

## Workflow Engine
- Validate transitions against code-defined transition map (shared in `TicketCore`).
...
- Summarize:
- Completed and newly created tickets.
...
- Emit `TicketEvent` within same transaction as source update.
